...
def filter_points(pts, z_min=-0.5, z_max=0.3, x_range=5.0, y_range=5.0):
    bbox = BoundingBox(-x_range, x_range, -y_range, y_range, z_min, z_max)
    return [p for p in pts if bbox.contains(p)]
...
class Cluster:
    ...
    def centroid(self): 
        n = len(self.points)
        return (sum(p[0] for p in self.points)/n, sum(p[1] for p in self.points)/n, sum(p[2] for p in self.points)/n)   
